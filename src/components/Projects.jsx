...
const Projects = () => {
  ...
  return (
    <section className={`p-24 ${isDarkMode ? 'bg-[#484148] text-white' : ''}`}>
      <h2 className="text-4xl font-medium font-inter ">{projectsData.title}</h2>

      <div className="flex gap-8 ">
        {/* First Project */}
        <div className={`mt-12 w-2/4 text-left px-12 py-12 bg-[#DDEEFE] relative rounded-xl ${isDarkMode ? 'bg-[#2D3235]' : ''}`}>
          ...
          <div className="flex gap-2 flex-wrap w-3/4 py-2 ">
            {Object.values(projectsData.firstProject.skills).map((skill, index) => (
              <p key={index} className={`font-playfair font-bold text-base border bg-[#FFFFFF] p-2 px-6 rounded-3xl ${isDarkMode ? 'bg-[#525252]' : ''}`}>{skill}</p>
            ))}
          </div>
        ...
        {/* Second Project */}
        <div className={`mt-12 w-2/4 text-left px-12 py-12 bg-[#D9F6F1] relative rounded-xl ${isDarkMode ? 'bg-[#495351]' : ''}`}>
